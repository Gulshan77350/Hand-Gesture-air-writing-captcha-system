...
# (e.g. a separate dev server / port). Serving it from this same FastAPI
# app, as done below, avoids the whole cross-origin problem.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# --- Serve the frontend from this same app/port -------------------------
# This avoids the file:// -> http://localhost cross-origin problem you'd
# hit opening index.html directly in a browser. Visit http://localhost:8000/
@app.get("/")
def serve_index():
